...
**169 张生成表也有对称的那一条**：算它们要读游戏，于是「表被手改、或改了生成器
...
文档里那些**由工具生成**的表格（169 张：doc 05 的 defines 表、doc 08 的目录统计表、
...
| 指标 | 值 |
|---|---|
| 测试 | **660 条**用例（657 通过 / 3 按条件跳过） |
| 覆盖率 | **约 87%**（86.9%，门禁 86% 由 pyproject 强制 + `v3 cov` 再按 7 个核心模块逐条设下限） |
...
├─ tools/
│   ├─ pdx/                  工具链核心包（39 个模块；解析部分纯标准库，cli.py 用 typer + rich）
│   ├─ tests/                测试（42 个测试文件 / 660 条用例）
│   ├─ prof/                 性能剖析
...
| 新东西 | 解决什么 |
|---|---|
| `v3 tables --offline` + 快照的 `doc_tables` 域 | CI 上（没有游戏）也能守住 169 张生成表 |
| `v3 cache`（解析磁盘缓存） | 全树解析 65 秒 → 读回 2.8 秒；16 个 xdist worker 共享，且 worker 只读不写 |
| `v3 cov`（覆盖率门禁） | 整体 86% 之外，再给 7 个核心模块各设下限 —— 整体数字会掩盖「大模块退化、小模块补测」 |
...
| 项 | 数值 | 说明 |
|---|---:|---|
| 知识库正文总量 | 1,199,650 字符 | 全部 21 篇（20 篇主题文档 + 索引） |
