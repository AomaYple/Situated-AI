...
文档里那些**由工具生成**的表格（149 张：doc 05 的 defines 表、doc 08 的目录统计表、
...
机械表只许减少，现为 19 张；余额的终点不是 0，而是那张「刻意不生成」清单的长度）。

### 实测

| 指标 | 值 |
|---|---|
| 测试 | **554 条**用例（551 通过 / 3 按条件跳过） |
| 覆盖率 | **约 88%**（88.05%，门禁 86%，由 pyproject 强制；xdist 下末位有 ±0.2% 抖动） |
...
├─ tools/
│   ├─ pdx/                  工具链核心包（28 个模块；解析部分纯标准库，cli.py 用 typer + rich）
│   ├─ tests/                测试（34 个测试文件 / 554 条用例）
│   ├─ prof/                 性能剖析
