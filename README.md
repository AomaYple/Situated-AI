...
文档里那些**由工具生成**的表格（119 张：doc 05 的 defines 表、doc 08 的目录统计表、
doc 19 的根目录与路径表、doc 04/14/15/16/17 那几族统计表）走另一条路：`v3 tables`
直接重算并逐行比对，不一致就退出码 1 —— 所以它们不可能过期，也不该手改。
「哪些表已经有人管」本身也有余额看守（`tools/tests/test_inventory.py`：无人看守的
机械表只许减少，现为 44 张；余额的终点不是 0，而是那张「刻意不生成」清单的长度）。

### 实测

| 指标 | 值 |
|---|---|
| 测试 | **538 条**用例（535 通过 / 3 按条件跳过） |
...
├─ tools/
│   ├─ pdx/                  工具链核心包（26 个模块；解析部分纯标准库，cli.py 用 typer + rich）
│   ├─ tests/                测试（32 个测试文件 / 538 条用例）
│   ├─ prof/                 性能剖析
