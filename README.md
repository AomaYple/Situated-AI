...
文档里那些**由工具生成**的表格（107 张：doc 05 的 defines 表、doc 08 的目录统计表、
doc 19 的根目录与路径表、doc 04/14/16/17 那几族统计表）走另一条路：`v3 tables`
...
| 指标 | 值 |
|---|---|
| 测试 | **521 条**用例（518 通过 / 3 按条件跳过） |
| 覆盖率 | **约 88%**（门禁 86%，由 pyproject 强制；xdist 下末位有 ±0.2% 抖动） |
...
├─ tools/
│   ├─ pdx/                  工具链核心包（25 个模块；解析部分纯标准库，cli.py 用 typer + rich）
│   ├─ tests/                测试（30 个测试文件 / 521 条用例）
│   ├─ prof/                 性能剖析
...
| 项 | 数值 | 说明 |
|---|---:|---|
| 知识库正文总量 | 1,137,100 字符 | 全部 21 篇（20 篇主题文档 + 索引） |
