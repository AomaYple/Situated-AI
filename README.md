...
把游戏与 mod 中**所有可脚本化的内容**解析成结构化数据，产出 7 份产物：

| 产物 | 内容 |
|---|---|
| `游戏本体.json` | 各目录的条目、字段、使用频次等**统计** |
...
| `snapshots/*.json` | 版本快照，升级时 diff 出字段增删 |
...
├─ docs/audits/              逐文件审计报告（重构前快照，历史存档）
├─ research/official-docs/   92 篇游戏自带官方 .md 的逐字镜像（Paradox 版权，见「授权」）
├─ tools/
│   ├─ pdx/                  工具链核心包（17 个模块，纯标准库）
│   ├─ tests/                测试（21 个测试文件 / 423 条用例）
│   ├─ prof/                 性能剖析
