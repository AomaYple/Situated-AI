...
| 指标 | 值 |
|---|---|
| 测试 | **735 条**用例（732 通过 / 3 按条件跳过） |
| 覆盖率 | **90.0%**（门禁 86% 由 pyproject 强制 + `v3 cov` 再按 7 个核心模块逐条设下限） |
...
├─ tools/
│   ├─ pdx/                  工具链核心包（41 个模块；解析部分纯标准库，cli.py 用 typer + rich）
│   ├─ tests/                测试（48 个测试文件 / 735 条用例）
│   ├─ prof/                 性能剖析
...
| 项 | 数值 | 说明 |
|---|---:|---|
| 知识库正文总量 | 1,281,126 字符 | 全部 21 篇（20 篇主题文档 + 索引） |
