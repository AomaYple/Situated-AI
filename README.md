...
机械表只许减少，**现为 0**；散文数字那一面从 115 降到 61，剩下的每一类的「为什么不该由工具算」都写在 `test_inventory.py` 的模块注释里）。

### 实测

| 指标 | 值 |
|---|---|
| 测试 | **577 条**用例（574 通过 / 3 按条件跳过） |
| 覆盖率 | **约 88%**（87.84%，门禁 86%，由 pyproject 强制；xdist 下末位有 ±0.2% 抖动） |
