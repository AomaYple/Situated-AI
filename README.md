...
[`tools/`](tools/) —— Python 实现的 PDX 脚本全量分析与信息提取工具链。
...
| 产物 | 内容 |
|---|---|
| `游戏本体.json` | 各目录的条目、字段、使用频次等**统计**（约 8.5 MB） |
...
另有一条**独立**的版本快照线（`v3 snapshot create`，落在 `tools/out/snapshots/`）：
捕获某个版本的全部 mod 相关信息，游戏升级后 `v3 snapshot diff` 一次就能看出
Paradox 增删了哪些字段。它约 41 MB/份，同样在 gitignore 范围内 —— 也就是
**不随仓库分发**，用途是同一个工作副本的跨版本比对。
