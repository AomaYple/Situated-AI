...
CI 跑 `v3 verify --from-snapshot`（真值来自**入库的离线真值**：精简快照 + 官方文档清单，覆盖其中 40 条）。
两条路径共用同一份断言注册表与同一个漂移扫描，不会出现「测试过了但工具没发现」。

文档里那些**由工具生成**的表格（166 张：doc 05 的 defines 表、doc 08 的目录统计表、
...
| 指标 | 值 |
|---|---|
| 测试 | **582 条**用例（579 通过 / 3 按条件跳过） |
| 覆盖率 | **约 88%**（87.8%，门禁 86%，由 pyproject 强制；xdist 下末位有 ±0.2% 抖动） |
...
| 项 | 数值 | 说明 |
|---|---:|---|
| 知识库正文总量 | 1,172,504 字符 | 全部 21 篇（20 篇主题文档 + 索引） |
