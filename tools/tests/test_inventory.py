...
#:
#: 进度 125 → 82 → 58 → 44 → 29：doc 05 的两张「假脚本化」表、doc 04/16 的「用了多少次」各一张、
#: doc 14 整族 20 张字段表、doc 16 整族 23 张、doc 17 整族 24 张、doc 06 的 5 张 + doc 15 的 7 张、
#: **doc 04 的 15 张**（另加 §0.2 目录规模、两张参数汇总、localization 对照表 ——
#: 那四张原先没被判成「统计表」，一并接进来）。生成表总数 31 → 77 → 107 → 119 → 139。
#:
#: 余额的**终点**不是 0，而是 :data:`NOT_GENERATED` 的条数 —— 那里面每一条都写明了
#: 「为什么不能机械生成」。做完一批就把这个数往下调；**调高必须在提交信息里说明理由**。
UNGUARDED_TABLE_BUDGET = 29
