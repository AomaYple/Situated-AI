...
PROSE_NOT_COMPUTED: dict[tuple[str, int], str] = {
    ...
    (
        "17-角色科技与呈现.md",
        442,
    ): "§18.4「实测全库共 442 处 `GetCustom(...)` 调用」—— **口径复现不出**："
    "`.yml` 全文检索是 14,071 次（356 个不同键、602 个文件），只算 `english/` 是 567 次，"
    "`.txt` 里 0 次，含 `GetCustom(` 的行数是 10,590 —— 没有一个是 442。与 doc 06 §1.5.5 "
    "数据函数频次同理：得先把「全库」写清楚（哪些内容根、算调用还是算键）才谈得上断言",
}
