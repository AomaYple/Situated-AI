...
散文那面的口径（第二轮放宽过一次，理由写在 :func:`_unguarded_prose` 里）：
**纯粗体数字**，外加「共 / 合计 / 总计 / 有 / 仅 / 只 + 数字 + 中文数量词」——
数量词含 **个 / 行 / 条 / 次 / 处 / 项 / 张 / 篇 / 份**。
早先只认「个」，于是「共 682 行」（doc 04 §4.6）整整漏了一个游戏版本：
单位是**证据的形式**，不是数字的性质。

**刻意不纳入的形态**（写下来是为了让口径有边 —— 否则每放宽一次都会翻出新数字，
这件事就永远收不了尾）：

* **中文数字与模糊量词**（「十几处」「约 40 行」「上百个」）—— 没有可核对的确定值，
  收进来只会得到一堆「近似」的断言；
* **区间与并列**（「5–20 KB」「2 个 / 3 个」）—— 断言只能钉一个数，
  硬拆成两条会改变作者的原意；
* **版本号、章节号、日期**（`1.14.3` / `§2.6` / `1836`）—— 不是统计量，
  而且点分数字本身已被漂移扫描整段跳过（见 `verify._DOTTED_NUM_RE`）；
* **文件名与键名里的数字**（`dlc018` / `1848_popular_radical`）—— 那不是「数量」，
  而是标识符的一部分；
* **表格单元格内的数字** —— 表格那面由 :data:`UNGUARDED_TABLE_BUDGET` 与
  `v3 tables` 单独看守，两边各管一段、不重复记账。

换句话说：**口径到此为止**。往后新增的散文数字，要么落在这个范围里
（那就必须补断言或写明理由，否则这条预算会红），要么属于上面几类
（那就不算欠债）。两种都答得出来，就不会再有「又漏了一类」的意外。
...
**散文**那一面是同一个终点形态：每个还没被 `v3 verify` 看守的数字，都必须能在
:data:`PROSE_NOT_COMPUTED` 里读到「为什么它不该由工具算」。那张表只收三类：

1. **另一台机器**（doc 02 / 06 / 12 / 14 / 索引页）—— 统计对象是**本机订阅的
   Workshop mod**。换台机器、退订一个 mod 就变：这不是「数字过期」，
   而是「这份快照描述的是另一台机器」。与 :data:`NOT_GENERATED` 里那几张表同理。
2. **原文如此**（doc 04 的 4 条、doc 07 的 512 / 41 / 20）—— 引用官方 `.md` 里
   写着的常量与清单。它们是**引用**而不是测量，改了就不再是引用；
   要核对只能核官方文件的 sha256（另有测试）。
3. **算例与勘误**（doc 04 / 07 的算例、doc 05 的 6125 / 6121）—— 算术推演的
   结果、以及「本节曾写错成什么」。它们存在的意义就是记录推导过程与历史错误，
   **天生没有期望值**；给它们补断言等于把「曾经错过的数」也当成事实。

判断标准始终是同一句：**是口径说不清，还是这个数字根本不该由工具拥有？**
「懒得做」不是理由 —— 这一轮就是靠这句话把一大批原先「说不清」的数字
（doc 05 的 jomini 未接管文件、doc 16 的 33 个目录、doc 17 的逐目录键数…）
变成了断言：它们不是不该算，只是还没人去算。

换句话说：终点不是「散文里一个数字都没有」，而是**剩下的每个数字都答得出
「为什么它不该由工具算」** —— 而这份清单本身也要看守（见
:func:`test_剩下的散文数字都写明了理由`：登记了却已不存在的数字必须删掉，
否则清单就从「理由」退化成「免责声明」）。
"""
...
#:
#: 进度 133（基线）→ 115 → 61 → 57 → 49 → 34 → 0（第一版口径）→ 24（放宽口径后）→ **0**：
#: 三轮把「口径唯一、可机械复算」的散文数字**逐批登记成了 `v3 verify` 断言**
#: （断言表 81 → 135 → 141 → 151 → 207 → 226 条），于是它们从「无人看守」变成「每次核验」：
#:
#: * doc 05/06/08：jomini 未接管的 15 个 defines 文件、没写 `decimals` 的 31 个键、
#:   `fonts.font` 的 53 个 `languages`、texticon 432、`.yml` 去重名 1,855 …；
#: * doc 16：33 个目录的 205 / 182 / 23、官方 `.md` 漏掉的 21 个字段与 6 个 kind、
#:   `strategic_regions` 的 34 / 36、`state_regions` 的 traits 517 …；
#: * doc 03/14/15/17/18/19：`ai_script_values` 的 703 / 33 / 75 / 64、买包 4 / 15 / 99、
#:   ideologies 的 35 / 26 / 9、`checksum_manifest` 的 5 + 1、doc 17 的 955 / 11,705 /
#:   1,037 / 614 / 93 / 97 / 317 / 343 / 1,407 …。
#:
...
#: 还剩 24 个这样的数字，四类理由（另一台机器 / 原文如此 / 算例与勘误 / 口径未写明）
#: 都在那张表里。
UNGUARDED_PROSE_BUDGET = 0
...
PROSE_NOT_COMPUTED: dict[tuple[str, int], str] = {
    ...
    ("README.md", 4769): "索引页 §3「对 23 个 mod 共 4,769 个相对路径」—— 本机 mod 侧统计",
    ("03-AI系统.md", 49): "§4 里「1.14.2 时同口径是 49 个」—— **版本对照用的历史值**。"
    "同一句的 1.14.3 值（行正则 64 / AST 75）都有断言看守；49 属于「曾经是多少」，"
    "与 doc 05 的 6125 / 6121 同一类：记录的是版本差，不是当下的事实",
    ("17-角色科技与呈现.md", 178): "§13.2 说明段里的「178 是 1.14.2 旧值」—— 同上，"
    "版本对照用的历史值。1.14.3 的 179 有断言看守（chr.tech_definitions），"
    "保留 178 是为了让读者看懂「差 1 之谜」其实只是版本差",
    # ── 放宽口径（2026-09：从「共 N 个」扩到 行/条/次/处/项）后需要理由的 ──
    ("03-AI系统.md", 43): "§4「距其块首（第 170 行）有 43 行」—— 这是**排版度量**：说明"
    "「解析时不能用固定行窗口」。它与 doc 05 那些表的「起始行」列同性质（改一行就全废），"
    "机械重算只会制造噪声",
    (
        "04-脚本系统.md",
        1,
    ): "§x「检索 md 中的示例名 → 仅 1 处命中」—— 这是**文档内部检索的一次记录**："
    "它要说明的是「官方 md 里只有这一处示例」，数的是 md 的正文而不是游戏数据",
    ("07-官方文档索引.md", 3): "§OOB「仅 3 处标注旗舰」—— **原文如此**：数的是官方 `1836_oob.md`"
    "里标了旗舰的条目（该文档是史实编制清单，不是游戏数据；要核对只能核官方文件的 sha256）",
    (
        "11-历史初始状态与AI策略分配.md",
        3,
    ): "§x 代码块里的「# 这个 else 分支同样有 3 条 set_strategy」"
    "—— **原文如此**：那是**官方脚本注释**的逐字引用，改它就不再是引用",
    ("16-外交军事与地图.md", 2): "§x「`contestion_type`（9 个值，但有 2 处差异）」—— 2 是作者对"
    "「官方 `.md` 的取值 vs 实测取值」的**逐行比对结论**（与 NOT_GENERATED 里 doc 17 基因分组同理："
    "两组清单都已由 `v3 tables` 逐行看守，差异标记是人写的判读）",
    # ── 第四类理由：**取证数字**（2026-09 加的一条登记口径）──────────
    #
    # 与前三类的区别：它们**是**工具算出来的（`v3 evidence` / `v3 unverified` /
    # `v3 prefixes` / 文件级统计），只是**不该被当成承诺**：数字描述的是
    # 「这一份安装 / 这一份语料 / 这一个二进制」的现状，Paradox 每次更新都可能变，
    # 而它们要支撑的判断是「原版有没有在用这个键 / 这个字段」这类**定性**结论。
    # 真正被承诺的量另有看守：目录条目与字段由断言表核验，版本变化由
    # `env.caligula_rev` 一族指纹在看（游戏一升级 `v3 verify` 就会红）。
    #
    # 换句话说：把用法次数写成断言，只会让每次游戏升级都因为「某个键多用了几次」
    # 而红一片，却答不出任何有意义的回归问题。
    (
        "04-脚本系统.md",
        49,
    ): "§6.2 取证数字：`cooldown` 在 `common/scripted_buttons` 下的用法次数"
    "（复算：`v3 evidence cooldown --dir common/scripted_buttons`）—— 支撑的是"
    "「原版确实在用这个字段」这一判断，不是被承诺的量",
    ("04-脚本系统.md", 98): "同上，同一次取证的另一半：`selected` 的用法次数"
    "（复算：`v3 evidence selected`）",
    ("04-脚本系统.md", 160): "§7.3 取证数字：`factor` 的用法次数（复算：`v3 evidence factor`）"
    "—— 用来回答「`factor` 是否只能用在 scripted_modifier 里」（结论：不是）",
    (
        "04-脚本系统.md",
        161,
    ): "§12.4 引擎侧取证：`victoria3.exe` 里 `<键名>_cw_duplicate_compat` 策略名的个数"
    "（复算：`v3 evidence --exe-grep _cw_duplicate_compat`）—— **线索层**：读的是"
    "「这一份二进制里有哪些名字」，且 `--exe-grep` 默认只列前 40 条、此处已显式取全量",
    ...
    (
        "02-Mod结构与加载.md",
        27723,
    ): "§5.1.2 与版本提示里的 `game\\` 全树文件数 —— **安装树规模**：数的是"
    "「本机那个文件夹里有什么」，含任何被外部程序丢进去的文件（实测被看图工具的两个"
    "缓存文件顶到 27,725，移出后回到 27,723）。与 `tree.game` 断言是**同一个量**，"
    "属「随本机安装状态漂移」的一类，故不在散文层再登记一条断言",
    (
        "02-Mod结构与加载.md",
        27725,
    ): "版本提示里记下的**曾经出现过的错值**（外部程序缓存文件造成的 27,725）—— "
    "与 doc 05 的 6125 / 6121 同类：它存在的意义就是记下「当时为什么会红」，"
    "天生没有期望值",
    (
        "15-政治人口与社会.md",
        0,
    ): "§11.6 更正段与版本提示里的**命中数 0**（`pop_types` 里没有 `popneed_*` 引用、"
    "12 个目录的官方 `.md` 数为 0、六前缀在 1.14.3 仍 0 命中）—— 「0 命中」本身就是"
    "取证结论，且 `v3 evidence` 的 ① 类只扫 `.txt`/`.gui`，0 只代表「这两类里没有」",
    (
        "06-本地化与界面资源.md",
        4914,
    ): "§6.3 的**勘误记录**：DDS 普查早期手写脚本把「未压缩 32bit」记成 4914，"
    "且各类相加比可解析总数少 29 —— 这个数字的意义就是记下「当时错成什么」，"
    "天生没有期望值（正确值 4943 见下一条）",
    (
        "06-本地化与界面资源.md",
        4943,
    ): "§6.3 语料计数：`fourCC` 为空的未压缩 32bit `.dds` 个数 —— 由 `v3 assets` 现算、"
    "`v3 tables` 逐行看守（那张表本身就是生成的），散文里出现只是因为它要对照"
    "那个错值解释差在哪",
    (
        "06-本地化与界面资源.md",
        0,
    ): "前言与 §1 的**命中数为 0**（`£`+字母、`$Nbsp$`、`$NEWLINE$`、行尾裸 `|`/`>` 等）"
    "—— 「0 命中」本身就是取证结论（复算：`v3 evidence --exe-grep <字样>` 或全树正则），"
    "且 ① 类只扫 `.txt`/`.gui`，所以 0 的含义是「这两类里没有」，不是「不存在」",
    (
        "06-本地化与界面资源.md",
        5,
    ): "§4.4 引用官方 `accessories\\accessories.md` 的**行数**—— **原文如此**："
    "数的是官方文档本身（与 doc 07 的 512 / 41 / 20 同类），不是游戏数据里的量",
    ("06-本地化与界面资源.md", 11): "§1 语料计数：`fonts.font` 的 `languages` 块数、"
    "`jomini` 词条文件数、`modifiers\\` 下的语言数 —— 都是**这一份安装**的目录事实，"
    "随 DLC 与版本变（与本节其余目录/文件计数同一份语料）",
    ("06-本地化与界面资源.md", 23): "§1.7 本机快照：本机 workshop 订阅的 23 个 mod —— "
    "换台机器、退订一个 mod 就变（与 doc 02 / 12 / 14 里那几条同一个理由）",
    ("06-本地化与界面资源.md", 44): "§3 / §6 语料计数：`gfx\\interface\\` 的子目录数、"
    "`city_types\\` 的文件数 —— 与上面同一份安装语料",
    ("06-本地化与界面资源.md", 77): "§1.2 语料计数：`languages.yml` 里标 1 / 标 0 的条目数"
    "（77 / 44，总数 121 见下）—— 本机这一份文件的统计",
    ("06-本地化与界面资源.md", 121): "§1.2 与 §4 语料计数：`languages.yml` 的条目总数"
    "（11 块 × 11 条）与 `[Nbsp]` 出现的文件数 —— 同一份安装语料",
    (
        "06-本地化与界面资源.md",
        705,
    ): "§5.1 的**勘误记录**：`fonts.font` 的行数曾写 705（错的），实为 743 —— "
    "这个数字的意义就是记下「曾经错成什么」，天生没有期望值"
    "（与 doc 05 的 6125 / 6121 同类）",
    ("06-本地化与界面资源.md", 743): "§5.1 / 附录 A 语料计数：`fonts.font` 的实际行数"
    "（四种独立口径一致）—— 随版本变的本机语料事实，同 11 / 44 那一类",
}
...
def _unguarded_prose() -> dict[str, set[int]]:
    ...
    #: 「共 N 个」**以及**「共 N 行 / 条 / 次 / 处 / 项」。
    #:
    #: 早先只认「个」——于是 doc 04 §4.6 的「共 682 行」整整活了一个游戏版本：
    #: 它不是粗体、单位又不是「个」，从口径里**整个漏掉**（doc 03 早就改成 703 了）。
    #: 单位是**证据的形式**，不是数字的性质：同一个量写成「1311 行」和「共 1311 个」
    #: 一样该有人看守。所以口径按「中文数量词」而不是按某一个词来定。
    cnt = re.compile(
        r"(共|合计|总计|有|仅|只)\s*\*{0,2}([\d][\d,]*)\s*\*{0,2}\s*(?:个|行|条|次|处|项|张|篇|份)"
    )
