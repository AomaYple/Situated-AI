...
PROSE_NOT_COMPUTED: dict[tuple[str, int], str] = {
    ...
    (
        "06-本地化与界面资源.md",
        11293,
    ): "§6.6 资产语料普查：`game\\gfx\\**\\*.dds` 里成功读到 128 字节头的文件数 —— "
    "每次美术 / DLC 更新都会变，且这轮是**一次性全量扫描**（`v3 evidence` 只管 "
    "`.txt`/`.gui`，尚未有脚本化的 DDS 普查工具）；把语料规模写成断言答不出回归问题",
    ("06-本地化与界面资源.md", 11294): "同上次普查的**分母**：找到的 `.dds` 总数"
    "（与 11293 差 1 个头部过短的文件）—— 同一条理由",
}
