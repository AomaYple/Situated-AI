...
整个文件**只有一个**顶层命名空间块 `NAI`，起始于第 1 行，包含 **1013** 个参数，全部为标量，无内联列表、无嵌套块。文件共 1307 行，全文中 `= {` 只出现 1 次（即第 1 行的 `NAI = {`）。【提取】
...
也就是说：`NAI` 是一个**完全扁平的、1013 条键值对的大块**，所有 AI 相关 define 都平铺在其中，靠命名前缀和注释分组，而不是靠嵌套命名空间。
...
由于 `NAI` 是完全扁平的 1013 条键，实际阅读时需要靠**命名前缀**分组。下表按「第一个下划线之前的部分」机械分组，按参数数降序排列。
...
下表由脚本从 `GAME\common\defines\00_ai.txt` 按**文件中的原始出现顺序**机械导出（第 2 行至第 1306 行），两列排版。全部 1013 个，无遗漏、无编造；`NAI` 块内**无重复键**（脚本校验：1013 条 → 1013 个去重名）。
...
| AI define 在哪个块？ | **`NAI`，且只有一个块，共 1013 个参数，全部扁平**（`00_ai.txt`，1307 行，147 KB） |
