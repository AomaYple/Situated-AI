...
> **版本提示（精确清单）**：本机游戏是 **1.14.4 (Ice Tea)**，但**本文的数字不是游戏版本属性，而是本机 mod 快照**（装了哪些 mod 决定一切）——这句限定比「采集于哪个版本」更要紧，故先说。原先那句笼统的「统计采集于 1.14.2」按类拆开：
> - **没有生成表**：`v3 tables` 的登记表里**没有本篇**（本篇 **0 张**生成表），因此不存在「表已重算」这回事；数字靠 `mod.*` / `mods.*` 断言与本节复算命令看守。
> - **已是 1.14.4 —— 断言已核验**：本篇 **3 条**断言（`mod.total`、`mod.files`、`mods.ig_files_doc12`）随 `v3 verify` 现场重扫，本次 **234/234 通过**（含本篇）。
> - **已复测（本机快照，1.14.3 现场）**：23 个 mod、4,796 个内容文件、去重后 4,769 条相对路径、覆盖原版 107 个文件 + 新增 4,689 个；§1 的「使用率」表逐行复算一致（`localization` 20、`common` 19、`thumbnail.png` 19 + `Thumbnail.png` 4 = 23、`gfx` 10、`events` 6、`gui` 2、`map_data` 1、`fonts` 1、`music`+`sound` 1、`dlc`+`dlc_metadata` 1）；§2 的逐 mod「覆盖/新增」表 10 行也逐行对上（50/1817、16/17、11/29、10/137、7/130、4/819、3/58、3/27、2/31、1/2，其余 13 个 mod 均为 0）；§9 第 2 条的 8 条重名路径与它们的计数（`thumbnail.png`×19、`Thumbnail.png`×4、`简介.txt`×2、`README.md`×2、`.idea/*`×4×2）也逐条对上。
> - **⚠️ 曾经错成 24 / 4,828 的那一版，原因不是「用户多订阅了一个 mod」**：那第 24 个是**我们自己部署进用户 mod 目录的真 mod**（跑 A/B 时装的），`pdx.mods.discover_mods()` 当时只排除 `zz_probe_` 前缀，于是把它当成了订阅项 —— 多算了它那 51 个文件。同一时期用户订阅的 `2880069248`（牛奶汉化）也**真的更新过**（+19 个本地化条目），两个变化叠在一起，所以两个数字都要重算、不能只做减法。根因与修法见 `pdx.mods.OWN_MOD_ID_PREFIX` 与 `backlog.md` **B83**；这次已复测，本节所有数字都以 23 个 Workshop mod 为准。
> - **版本分布已与 §7 对齐**：§9 第 1 条给的那四档分布是本次实测值，§7 那张表原先写的 `1.13*` 与「空」两格是过期值，已同步。该表**没有生成器看守**，改由 `test_mods.py::test_doc12_版本分布表与现场一致` 逐格核对现场（两张表从此不许再互相矛盾）。
...
**【实测】** 跨 23 个 mod 共 **4,796<!--claim:mod.files--> 个内容文件**（各 mod 的 `metadata.json` 不计入），
去重后是 **4,769 个相对路径**：
...
**这解释了本文第 2 节开头那个反直觉的观察**：为什么 4,769 个相对路径里零重叠、23 个 mod 能共存 ——
...
> 「文件数」这一列**含各 mod 自己的 `metadata.json`**（所以比 `v3 analyze` 里那份不含元数据的计数大一）。
> 本表只用于横向比较形态，不是断言 —— 精确的逐 mod 文件数见 `tools/reports/mod分析.md` §三。
...
**【实测】** 各 mod 声明的 `supported_game_version`（**本机快照**，复算同 §9 第 1 条）：

| 声明值 | mod 数 | 实例 |
|---|---|---|
| `1.13*` | 9 | Ultra Historical 系列 |
| `1.13.*` | 4 | More Character Traits、Kuromi's AI、War Is Politics、Shocks to the System |
| `1.*` | 1 | HANE Music |
| **空** | 9 | 汉化 mod、Dynamic Names 等 |
...
| 2 | 【未确认】覆盖同一原版文件时，多个 mod 的最终生效规则 | ③ 23 个 mod 共 **4,796** 个内容文件、去重后 **4,769** 个相对路径；**被 ≥2 个 mod 命中的路径只有 8 条**，且**没有一条在内容目录**（`common/`、`localization/`、`events/`、`gfx/`、`gui/`、`map_data/` 下 0 条）：`thumbnail.png`×19、`Thumbnail.png`×4、`简介.txt`×2、`README.md`×2、`.idea/{.gitignore,misc.xml,modules.xml,workspace.xml}`×2 —— 即 §2 的「零重叠」在**内容路径**上成立（本机快照，复算同第 1 条）→ 「谁最终生效」**无实例可判** | 【未确认】待实测 → 配方 D |
| 3 | `localization/` 下具体改哪些文件（未逐 mod 展开） | **已答（本机快照，逐 mod 文件条目数）**：20/23 个 mod 带 `localization/`，条目合计 **1,475**（含跨 mod 重复）。逐 mod：`2880069248`（牛奶汉化）**1075**、`2918521358` 88、`3507724904` 72、`3007678964` 55、`3626012595` 33、`3018606449` 33、`3132637630` 26，其余 ≤11；`3227982912`（Kuromi's AI）、`3647628156`、`3117838814`（萝莉体）为 **0**（它只给 `gfx`/`common`）。复算：`overrides + additions` 里 `localization/` 前缀计数 | 已答 |
| 4 | 各 mod 的 `common/` 具体改了哪些键（仅统计了文件级） | **已答（口径从文件级升到条目级，本机快照）**：23 个 mod 在**全部目录**下共 **10,379 条新增条目 + 807 条改原版条目**（`ModInfo.added_entries` / `touched_vanilla`，键是**相对路径**、不限 `common/`）。文件数/条目数最大的几个（**「文件」这一列是 `common/` 下的文件数**，与上面的条目口径不同，别混用）：`3507724904` **197 文件 / 5,872 条目**（改原版 10）、`3032185644` 79 / 1,317、`3459869359` 6 / 1,045（改原版 **675**，是本机最激进的一个）、`3656697666` 11 / 591、`3626012595` 29 / 463、`2880069248` 0 文件 / 450 条目。带前缀的键另有**样例**（`ModInfo.prefix_samples`，最多 400 条/mod；按目录的用量见 `v3 prefixes`）。⚠️ 逐键全清单体量大，未抄进文档 —— 但口径已可一条命令现算 | 已答 |
