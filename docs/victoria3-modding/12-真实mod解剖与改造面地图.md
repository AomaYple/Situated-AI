...
> 本文对**本机已订阅的全部 24<!--claim:mod.total--> 个 Steam Workshop mod** 做机械解剖，回答一个核心问题：
...
**【实测】** 统计每个顶层目录被多少个 mod 触及（共 24 个 mod）：
...
**【实测】** 跨 24 个 mod 共 **4,828<!--claim:mod.files--> 个内容文件**（各 mod 的 `metadata.json` 不计入），
去重后是 **4,750 个相对路径**：
...
**零重叠。** 这正是 24 个 mod 能同时启用而互不冲突的根本原因。
...
**六个前缀在 24 个 mod 中的出现次数**（独立复核，规则：行首 `^前缀:键名 = {`）：
...
**这解释了本文第 2 节开头那个反直觉的观察**：为什么 4,750 个相对路径里零重叠、24 个 mod 能共存 ——
